...
class IMUCNN_LSTM(nn.Module):
    def __init__(self, config):
        super(IMUCNN_LSTM, self).__init__()
        ...
        # Update LSTM input size to feature_dim (not divided)
        self.lstm = nn.LSTM(feature_dim, lstm_hidden_dim, num_layers=num_layers, batch_first=True)
        
        # Fully connected layers
        self.fc1 = nn.Linear(lstm_hidden_dim, feature_dim)
    ...
    def forward(self, data):
        ...
        # Prepare for LSTM: Shape (B, T', feature_dim)
        x = x.transpose(1, 2)                 # Shape: B x (T/2) x feature_dim

        # LSTM expects input of shape (batch, seq_len, input_size)
        lstm_out, _ = self.lstm(x)            # Shape: B x (T/2) x lstm_hidden_dim
        
        # Use the last output of LSTM for classification
        x = lstm_out[:, -1, :]                # Take the last time step's output: Shape: B x lstm_hidden_dim
